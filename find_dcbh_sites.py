...
def cooling_condition(t_cool,prog_counts, times):
     no_index = np.where(t_cool < times*SEC_PER_MYR)
...
def draw_jlw_base():
    upperbound =7
    final = quad(calc_N, 0, upperbound, args=(np.log10(J_MEAN_BASE)))[0]
    x_vals, cdf_vals = [],[]
    for x in np.arange(0,upperbound,0.01):
        cdf_vals.append(quad(calc_N, 0, x, args=(np.log10(J_MEAN_BASE))) [0]/final)
        x_vals.append(x)
...
def draw_jlw_ratio(j_mean, x_vals, cdf_vals, N_draws = 1):
...
def main():
    f = sys.argv[1]
    tree_num = int(f.split("trees")[-1])
    lines_per_chunk = 6e6*10

    n_rows = int(lines_per_chunk * 1.1)
    print(n_rows, lines_per_chunk)


    start_index = 1

    for tree_chunk in range(100):

	    print("reading tree chunk ", str(tree_chunk))
	    ...
	    z = get_redshift("data/Snapshot_alist_mod")
	    ...
	    ds_write.to_csv('dcbh_files/tree'+str(tree_num)+'/chunk_'+ str(tree_chunk) +'_index'+str(start_index)+'_sitenum_'+str(total_dcbh_sites)+'.csv',index=True)
